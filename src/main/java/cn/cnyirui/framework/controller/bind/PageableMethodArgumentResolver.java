...
public class PageableMethodArgumentResolver extends BaseMethodArgumentResolver {
	...
	private String orderParameterName = DEFAULT_ORDER_PARAMETER;
	 ...
	 * used to back missing parameters. Defaults
	 ...
	 * pageParameterName
	 * 
	 * @param pageParameterName pageParameterName
	 */
	 ...
	 * sizeParameterName
	 * 
	 * @param sizeParameterName sizeParameterName
	 */
	...
	public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
		...
		Map<String, String[]> pageableMap = getPageableParameterMap(parameter, webRequest);
		Map<String, String[]> sortMap = getSortParameterMap(parameter, webRequest);
		String sortNamePrefix = getSortPrefix(parameter, true);
	 ...
	 * possible {@link Qualifier} if available or
	 * return the configured prefix otherwise.
	 *
	 * @param parameter
	 * @return
	 */
	private String getPagePrefix(MethodParameter parameter, boolean addPagePrefix) {
		String result = null;
		if (addPagePrefix) {
			result = pagePrefix;
		} else {
		...
		if (qualifier != null) {
			return new StringBuilder(((Qualifier) qualifier).value()).append("_").append(result).toString();
		}

		return result;
	}

	private String getSortPrefix(MethodParameter parameter, boolean addSortPrefix) {
		String result = null;
		if (addSortPrefix) {
			result = sortPrefix;
		} else {
		...
		Qualifier qualifier = parameter.getParameterAnnotation(Qualifier.class);
		if (qualifier != null) {
			return new StringBuilder(qualifier.value()).append("_").append(result).toString();
		}
	...
	private Map<String, String[]> getPageableParameterMap(MethodParameter parameter, NativeWebRequest webRequest) {
		Map<String, String[]> pageableMap = null;
		String pageableNamePrefix = getPagePrefix(parameter, false);
		String pageParameter = webRequest.getParameter(pageableNamePrefix + pageParameterName);
		if (pageParameter != null) {
			pageableMap = new HashMap<String, String[]>();
			pageableMap.put("pn", new String[] { pageParameter });
			pageableMap.put("size", new String[] { webRequest.getParameter(pageableNamePrefix + sizeParameterName) });
		} else {
			pageableNamePrefix = getPagePrefix(parameter, true);
			pageableMap = getPrefixParameterMap(pageableNamePrefix, webRequest, true);
		}
		return pageableMap;
	}

	private Map<String, String[]> getSortParameterMap(MethodParameter parameter, NativeWebRequest webRequest) {
		Map<String, String[]> sortMap = null;
		String sortNamePrefix = getSortPrefix(parameter, false);
		String sortParameter = webRequest.getParameter(sortNamePrefix + sortParameterName);
		if (sortParameter != null) {
			String orderParameter = webRequest.getParameter(sortNamePrefix + orderParameterName);
			sortNamePrefix = getSortPrefix(parameter, true);
			sortMap = new HashMap<String, String[]>();
			String[] fields = sortParameter.split(",");
			String[] orders = null;
			if (orderParameter != null) {
				orders = orderParameter.split(",");
			} else {
				orders = new String[fields.length];
			}
			for (int i = 0; i < fields.length; i++) {
				String order = null;
				if (orders.length > i) {
					order = orders[i] == null ? "asc" : orders[i];
				} else {
					order = "asc";
				}

				sortMap.put(sortNamePrefix + i + "." + fields[i], new String[] { order });
			}
		} else {
			sortNamePrefix = getSortPrefix(parameter, true);
